  ...
  <div class="control-group flex flex-col">
    ...
    <div class="theme-selector relative">
      ...
      <div v-if="isDropdownOpen"
        class="absolute bottom-full left-0 right-0 mb-1 bg-[var(--input-bg-color)] border border-[var(--input-border-color)] rounded-md shadow-lg z-50 max-h-80 overflow-y-auto">
        <div v-for="(category, categoryKey) in categorizedThemes" :key="categoryKey" class="theme-category">
          <div
            class="category-header px-3 py-2 text-xs font-semibold text-[var(--thumbnail-secondary-text-color)] bg-[var(--controls-bg-color)] border-b border-[var(--input-border-color)]">
            {{ t(`themeCategories.${categoryKey}`) }}
          </div>
          <div class="category-themes">
            <button v-for="(theme, key) in category" :key="key" @click="selectTheme(key)"
              ...
              <div class="theme-preview w-4 h-4 rounded border border-[var(--input-border-color)]"
                :style="{ backgroundColor: theme.colors['--thumbnail-bg-color'] }"></div>
              <span class="text-[var(--input-text-color)]">{{ t(`themes.${key}.name`) }}</span>
            </button>
          </div>
        </div>
      </div>
